...
def progressive_learning(vid_name, fea, label, top_rate, path):
    ...
    cluster_center_all_fea = {}
    for i in range(all_class):
        cluster_center_all_fea[i] = []
    cluster_center_all_fea_pos = {}
    for i in range(all_class):
        cluster_center_all_fea_pos[i] = []
    for i in range(fea.shape[0]):
        key = label[i]
        cluster_center_all_fea[key].append(fea[i])
        cluster_center_all_fea_pos[key].append(i)
    for i in range(all_class):
        # print(len(cluster_center_all_fea[i]))
        cluster_center_all_fea[i] = np.stack(cluster_center_all_fea[i], axis=0)
    ...
    # dists = []
    all_num = len(label)
    dists_min = np.zeros(len(label))
    for i in range(len(all_center)):
        key = i
        cluster_center = all_center[i]
        cluster_center = cluster_center.reshape(1, cluster_center.shape[0])
        # final_dist = re_ranking(cluster_center, cluster_center_all_fea[i])
        final_dist = []
        for j in range(len(cluster_center_all_fea[i])):
            final_dist.append(np.linalg.norm(cluster_center_all_fea[i][j] - cluster_center))
        # print(len(final_dist))
        dists_min[cluster_center_all_fea_pos[key]] = final_dist
        # dists_min.extend(final_dist)

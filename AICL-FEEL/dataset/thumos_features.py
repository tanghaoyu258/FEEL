...
class ThumosFeature(Dataset):
    def __init__(self, data_path, mode, modal, feature_fps, num_segments, len_feature,
                 sampling, seed=-1, supervision='weak', data_anno_path=''):
        if seed >= 0:
        ...
        split_path = os.path.join(data_anno_path, 'split_{}.txt'.format(self.mode))  # 换这个里边的视频名字
        print(os.path.abspath(split_path))
        ...
        anno_path = os.path.join(data_anno_path, 'gt.json')
        anno_file = open(anno_path, 'r')
        self.anno = json.load(anno_file)
        anno_file.close()

        class_dict_temp = dict((v, k) for k, v in class_dict.items())
        if data_anno_path != data_path:
            class_dict_temp = {}
            for i in range(100):
                class_dict_temp[i] = i
            self.class_name_to_idx = class_dict_temp
        else:
            self.class_name_to_idx = class_dict_temp
    ...
    def get_data(self, index):
        ...
        if self.modal == 'all':
            rgb_feature = np.load(os.path.join(self.feature_path[0], vid_name + '.npy')).astype(np.float32)
            flow_feature = np.load(os.path.join(self.feature_path[1], vid_name + '.npy')).astype(np.float32)

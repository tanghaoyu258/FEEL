...
def dump_pseudo_multilabel(num_of_cluster, cluster_res, anet_label, path):
    temp_json = anet_label
    ...
    for tmp_cls in range(num_of_cluster):  # 聚类的伪标签->视频名
        # print(cluster_res[tmp_cls])
        for tv in cluster_res[tmp_cls]:
            if tv not in cnt_dict:
                cnt_dict[tv] = 1
                for i in range(len(temp_json[tv]['annotations'])):
                    temp_json[tv]['annotations'][i]['label'] = tmp_cls
            else:
                if cnt_dict[tv] >= len(temp_json[tv]['annotations']):
                    temp_json[tv]['annotations'].append(temp_json[tv]['annotations'][0])
                    temp_json[tv]['annotations'][cnt_dict[tv]]['label'] = tmp_cls
                    cnt_dict[tv] += 1
                else:
                    temp_json[tv]['annotations'][cnt_dict[tv]]['label'] = tmp_cls
                    cnt_dict[tv] += 1
...
def dump_split_unlabeled(subset_index, path):
    anno_txt_file = os.path.join(path, 'split_unlabeled' + '.txt')
    file = open(anno_txt_file, "w", encoding='utf-8')
    subset_index.sort()
    for vid in subset_index:
        file.write(vid + '\n')
...
#        print(cluster_res[tmp_cls])
    if dump == True:
        ...
        all_key_list = video_2_action.keys()
        all_subset_index = list(all_key_list)
        subset_index_unlabeled = list(set(all_subset_index) - set(subset_index))
        dump_split_unlabeled(subset_index_unlabeled, path)
    ...
    # print(gt_label)
    # print(label_pred)
    label_pred_final = [[item] for item in label_pred]
    # from sklearn.preprocessing import MultiLabelBinarizer
    # mlb = MultiLabelBinarizer()
    # binary_gt_label = mlb.fit_transform(gt_label)
    # binary_label_pred = mlb.transform(label_pred_final)
    # print(binary_label_pred.shape)
    # print(binary_gt_label.shape)
    # print(label_pred_final)
...
def get_multilabel_cluster_performance(num_of_cluster, label_pred, subset_index, action_2_video, video_2_action, path, dump=False):
    cluster_res = {}
    for tmp_cls in range(num_of_cluster):
        #print(tmp_cls)
        cluster_res[tmp_cls] = []
    ...
    # print(label_pred)
    for i in range(len(label_pred)):
        label_pred[i] = list(label_pred[i])
        for j in range(len(label_pred[i])):
            tmp_cls = label_pred[i][j]
            file_name = subset_index[i]
            cluster_res[tmp_cls].append(file_name)
...
#        print(cluster_res[tmp_cls])
    if dump == True:
        dump_pseudo_multilabel(num_of_cluster, cluster_res, anet_label, path) # 改多标签
        dump_split_train(subset_index, path)
        all_key_list = video_2_action.keys()
        all_subset_index = list(all_key_list)
        subset_index_unlabeled = list(set(all_subset_index) - set(subset_index))
    ...
    # print(gt_label)
    label_pred_final = []
    for i in range(len(label_pred)): # 处理一下label-pred
        label_pred[i] = list(label_pred[i])
        label_pred_final.append(label_pred[i])
    # print(label_pred_final)
    label_pred_final = np.array(label_pred_final)
    label_pred_final = np.squeeze(label_pred_final)
    # print('!!!!!!!')
    from sklearn import metrics
    print("Adjusted rand score %.4f" % metrics.adjusted_rand_score(gt_label, label_pred_final))
    print("NMI %.4f" % metrics.normalized_mutual_info_score(gt_label, label_pred_final))
...
# if __name__ == '__main__':
    ...
    # estimator = KMeans(n_clusters=num_of_cluster, random_state=0)
    # estimator.fit_predict(subset_atten_fea)
    ...
    # dump_soft_cluster_2_action(soft_cluster_2_action)
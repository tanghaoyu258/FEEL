...
def get_train_label(anet_label):
    ...
    for tv in anet_label:
        if anet_label[tv]["subset"] != "train":
            continue
        training_index.append(tv)
        for i in range(len(anet_label[tv]["annotations"])):   # changed-to-multilabel
            tc = anet_label[tv]["annotations"][i]["label"]

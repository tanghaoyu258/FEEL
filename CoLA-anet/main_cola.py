...
def get_easy_act(net, cfg, test_loader, test_info, step, writter=None):
    ...
    if int(cfg.iter) == 0:
        for data, label, _, vid, vid_num_seg in test_loader:
            ...
            weighted_features = np.mean(easy_act, axis=0)
            ...
            subset_atten_fea.append(normalized_features)
            subset_index.append(vid[0])
        subset_atten_fea = np.concatenate(subset_atten_fea, axis=0)

        num_of_cluster = 100
        print(subset_atten_fea.shape)
        label_pred = cluster(num_of_cluster, subset_atten_fea)
        cluser_2_action, soft_cluster_2_action = get_cluster_performance(100, label_pred, subset_index, action_2_video,

...
class NpyFeature(data.Dataset):
    def __init__(self, data_path, mode, modal, feature_fps, num_segments, sampling,
                 class_dict, seed=-1, supervision='weak', data_anno_path=''):
        if seed >= 0:
            utils.set_seed(seed)
    ...
    def get_data(self, index):
        ...
        if self.modal == 'all':
            ...
            elif self.sampling == 'non':
                sample_idx = self.non_sampling(rgb_feature.shape[0])
            else:
            ...
            if self.sampling == 'random':
                ...
                sample_idx = self.non_sampling(feature.shape[0])
            else:
                raise AssertionError('Not supported sampling !')
    ...
    def get_label(self, index, vid_num_seg, sample_idx):
        vid_name = self.vid_list[index]
        anno_list = self.anno['database'][vid_name]['annotations'] # 换这个里边的标签

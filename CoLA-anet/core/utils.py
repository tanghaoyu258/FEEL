...
def result2json(result, class_dict):
    ...
    cluster_4_action_file = os.path.join(cfg.DATA_ANNO_TRAIN_PATH, 'soft_cluster_cls_names_anet12.json')#"misc/soft_cluster_cls_names_anet12_r_3.json"
    ...
    for i in range(len(result)):
        for j in range(len(result[i])):
            line = {'label': result[i][j][0], 'score': result[i][j][1],
...
def set_path(config):
    if config.MODE == 'train':
        config.EXP_NAME = 'experiments{cfg.iter}/{cfg.MODE}/easy_{cfg.R_EASY}_hard_{cfg.R_HARD}' \
        ...
        config.EXP_NAME = 'experiments{cfg.iter}/train/easy_{cfg.R_EASY}_hard_{cfg.R_HARD}' \
                          '_m_{cfg.m}_M_{cfg.M}_freq_{cfg.TEST_FREQ}' \
                          '_seed_{cfg.SEED}'.format(cfg=config)
        config.MODEL_PATH = os.path.join(config.EXP_NAME, 'model')
